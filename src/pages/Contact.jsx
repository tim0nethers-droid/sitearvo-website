...
const initial = { name: '', email: '', phone: '', company: '', projectTitle: '', country: 'India', service: '', budget: '', message: '' };
...
function CountrySelect({ value, onChange, error }) {
  ...
  return <label className="field"><span>Country *</span><select name="country" value={value} onChange={onChange} aria-invalid={!!error} aria-describedby={error ? errorId : undefined}><option value="">Select a country</option>{countryOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select>{error && <small id={errorId}>{error}</small>}</label>;
}
