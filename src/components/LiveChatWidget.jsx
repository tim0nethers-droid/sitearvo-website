...
export default function LiveChatWidget() {
  ...
  return <div className={`live-chat ${open ? 'is-open' : ''}`}>
    {open && <section className="live-chat-panel" aria-label="SiteArvo live chat">
      <header><div className="live-chat-agent"><span><AppIcon icon="user" aria-hidden="true" /></span><div><b>SiteArvo Live Support</b><small><i /> Replies in this chat</small></div></div><button type="button" onClick={() => setOpen(false)} aria-label="Close live chat"><AppIcon icon="close" /></button></header>
      {!token ? <form className="live-chat-start" onSubmit={start}>
        ...
        <button className="button" disabled={busy}>{busy ? 'Starting chat...' : <>Start Live Chat <AppIcon icon="message-circle" /></>}</button>
        ...
        <div className="live-chat-reference"><span>Conversation #{chat?.public_id}</span>{chat?.status === 'closed' && <button type="button" onClick={newChat}><AppIcon icon="arrow-left" /> Start new chat</button>}</div>
